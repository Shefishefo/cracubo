...
START = f'🙋‍♂ Добро пожаловать в бот -=EXTREME=-. Я создан для того, чтобы помогать тебе получать взаимных подписчиков на твой канал. Перед началом использования бота, обязательно прочитай небольшую [инструкцию по использованию бота, а также правила]({LINK_TO_INTRODUCTION_AND_RULES})'
UPDATE = '🔙 Возвращаемся в главное меню'
LITTLE_SUBCOIN_1 = f'❗️Для раскрутки вашего канала у вас должно быть минимум {LITTLE_SUBCOIN_TO_GET_SUBS} Akatsuki Coin!'
LITTLE_SUBCOIN_2 = '😳 Недостаточно Akatsuki Coin!'
...
def PROFILE(m):
    profile = f'*👤 Имя:* {m.from_user.first_name}\n*📟 ID:* `{m.from_user.id}`\n*💰 Баланс:* {user_balance(m.from_user.id)} *Akatsuki Coin 🤩*'
    return profile
...
def PARTNERS(username_of_bot, user_id, ref_count):
    message = f'❕*Приглашайте в бота друзей и знакомых* по своей реферальной ссылке и получайте по *2 Akatsuki Coin* за каждого.\n*👣 Количество рефералов:* {ref_count}\n*🧲 Ваша реферальная ссылка:* \nhttps://t.me/{settings.botname}?start={user_id}'
    return message
